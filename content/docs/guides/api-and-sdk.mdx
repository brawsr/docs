...
- TypeScript: [`@brawsr/sdk`](https://www.npmjs.com/package/@brawsr/sdk),
  for Node.js 22 and newer;
- Python: [`brawsr`](https://pypi.org/project/brawsr/), for Python 3.11 and
  newer.

Both SDKs default to `https://api.brawsr.io`. Use an explicit `baseUrl` in
TypeScript or `base_url` in Python only when connecting to another deployment.
See [Install](/docs/get-started/installation) for package commands and endpoint
configuration.
...
checkpoint returns when its captured state can be used. Rewind returns when the
restored browser is CDP-usable, which can be before bounded post-restore work is
terminal. Reconnect immediately for browser work; wait with `waitRewind` or
`wait_rewind` before the next lifecycle mutation on that session.
...
const restoredBrowser = await chromium.connectOverCDP(restoredConnection.endpointUrl, {
  headers: restoredConnection.headers,
});
const restoredPages = restoredBrowser.contexts()[0].pages();
```
...
with BrawsrClient() as brawsr, sync_playwright() as playwright:
    ...
    restored_pages = restored_browser.contexts[0].pages
```
...
Four story-driven examples with focused code excerpts are available in these docs:
[TypeScript rewind](/docs/examples/playwright-rewind-typescript),
[Python rewind](/docs/examples/playwright-rewind-python),
[TypeScript fork](/docs/examples/playwright-fork-typescript), and
[speculative branching](/docs/examples/speculative-branching).
...
the CDP-usable live session result; terminal post-restore work and
browser-library reconnection are separate. Service targets apply only when
explicitly stated in the brawsr service documentation.
